...
__device__ Vec vec_sample_unit(int i) {
  ...
  float pho = random_float(&seed) * 2 * 3.1415926535897931;   
...
__global__ void ray_trace_kernel(float* d_window, int width, int ray_num) {
  ...
  int ray_chunck = ray_num/gridDim.x ;
  for (int i = 0; i < ray_chunck; i+= blockDim.x * gridDim.x){
    ...
    Vec V, W;
    while(1){
      V = vec_sample_unit(blockIdx.x * blockDim.x + threadIdx.x);
      if (V.y == 0) continue;
...
int main(int argc, char* argv[]) {
  ...
  float* window = (float*) calloc(0.0, grid_len*grid_len*sizeof(float));
  float* d_window;
  size_t size = grid_len * grid_len * sizeof(float);
  cuchk(cudaMalloc(&d_window, size));
  cuchk(cudaMemcpy(d_window, window, size, cudaMemcpyHostToDevice));

  ray_trace_kernel<<<1,1>>>(d_window, grid_len, num_rays);
